<?php
/*
 * Plugin Name: Custom Post Types slider
 * Plugin URI: http://www.netmadness.net
 * Description: This plugin creates an slider of the thumbnail images on custom post types
 * Author URI: http://www.netmadness.net
 * Author: Luis Sierra
 * Version: 10.0.0
 * Licence: GPLv2
 *
 * This plugin is based on the original works of Julien Le Thuaut (MBA Multimedia) http://jltweb.info/
 * There is a description of what the original plugin does here:
...
class WpParallaxContentSlider
{
	...
	function __construct() {
		...
		add_action('add_meta_boxes', [$this, 'add_meta_box_field']);
		add_action( 'save_post', [$this, 'save_post_slideshow_selector'] );
	} // end constructor

	/**
	* This function adds the metabox checkbox to choose whether the user wants
	* to display the current post inside the slideshow
	*/
	public function add_meta_box_field()
	{		
		add_meta_box( 
	    	'in_slideshow',
	    	'En Slideshow', 
	...
	/**
	* This function saves the state of the custom meta box
	*/
	public function save_post_slideshow_selector($post_id)
	{
	    $is_valid_nonce_in_slide = ( isset( $_POST[ 'slide_nonce' ] ) && wp_verify_nonce( $_POST[ 'slide_nonce' ], 'in_slide' . basename( __FILE__ ) ) ) ? 'true' : 'false';    

	    // Exits script depending on save status
	    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) 
			return $post_id;
	    
	    if($is_valid_nonce_in_slide){
	        $value = isset($_POST['in_slideshow']) ? true : false;
	        update_post_meta( $post_id, 'in_slideshow',  $value );
	    }
	}


	/**
	* This function renders the html that displays the checkbox
	*/
	public function render_meta_box($post)
	{?>
		<input type="hidden" name="slide_nonce" value="<?php echo wp_create_nonce('in_slide' . basename(__FILE__)); ?>" />
        <?php $selected = get_post_meta($post->ID, 'in_slideshow', 1); ?> 
        <input type="checkbox" name="in_slideshow" id="in_slideshow" <?php checked( $selected, 1 ); ?>/>
        <label for="in_slideshow">Mostrar en Slideshow?</label><?php	
	}
	...
	public function get_parallax_content_slider( $category = '' )
	{
		...
		$outputDynamic .= $prlx_text_content === 'none' ? " no-content'>\n": "'>\n";						
		...
		while ( $myposts->have_posts() ) : $myposts->the_post();
			...
			switch ($prlx_text_content) {
				...
				$outputDynamic .= $this->get_article_slide( get_the_title(),
					...
					break;
				case 'none':
				$outputDynamic .= $this->get_article_slide( '', 
															'none', 
															get_permalink(), 
															$default_slide_image,
															$prlx_title_max_chars )."\n";
...
DYNAMICOUTPUT;
	...
	function get_article_slide( $title, $excerpt, $link_article, $url_image, $title_length, $alt_image = 'Alternative text' )
	{
		...
		if($excerpt === 'none'){
			// Slide output without content
			$outputSlide  = "<div class='da-slide'>"."\n";
			$outputSlide .= "<div class='da-img no-content'><img src='".$url_image."' alt='".$alt_image."' /></div>"."\n";
			$outputSlide .= "</div>"."\n";
		}else{
			// Slide output
			$outputSlide  = "<div class='da-slide'>"."\n";
...
<div class="wrap">
	...
	<form action="?page=wp-parallax-content-slider" method="POST">
		...
		<input type="hidden" name="prlx_slider_mode" value="dynamic">
		<input type="hidden" name="prlx_slider_bgincrement" value="50">
		<input type="hidden" name="prlx_slider_theme" value="silver">
		<input type="hidden" name="prlx_slider_first_slide" value="1">
					...
					<tr>
						...
						<td>
							<select name="prlx_slider_content_type" id="prlx_slider_content_type">
								<option value="artes" <?php if ( $prlx_slider_settings['content_type'] === "artes") echo 'selected="selected"'; ?>><?php _e( 'Artes', 'wp-parallax-content-slider' ); ?></option>
					...
					<tr>
						...
						<td>
							<select name="prlx_text_content" id="prlx_text_content">
								...
								<option value="none" <?php if ( $prlx_slider_settings['text_content'] === "none") echo 'selected="selected"'; ?>><?php _e( 'None', 'wp-parallax-content-slider' ); ?></option>
							</select>
